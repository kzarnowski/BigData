...
// MAGIC * `date_format()` - formatuje datę do stringa
...
var newDf = dataFrame.select($"*", year($"current_date").alias("year"), month($"current_date").alias("month"), dayofyear($"current_date").alias("dayofyear"))
display(newDf)                   
...
// MAGIC %md
// MAGIC Użycie funkcji

// COMMAND ----------

var df_1 = dataFrame.select(
  $"current_timestamp",
  date_format($"current_timestamp", "yyyy MM dd").as("yyyy MM dd"),
  date_format($"current_timestamp", "MMMM dd").as("MMMM dd"),
...
var df_3 = df_1.select(
  to_date($"yyyy MM dd", "yyyy MM dd").as("yyyy MM dd"),
  to_date($"MMMM dd", "MMMM dd").as("MMMM dd") // sets year to 1970
)
...
var df_5 = DataFrame.select(
  $"current_timestamp",
  from_utc_timestamp($"current_timestamp", "GMT+1"),
  from_utc_timestamp($"current_timestamp", "GMT+2")
)
...
var df_6 = DataFrame.select(
  $"current_timestamp",
  to_utc_timestamp($"current_timestamp", "GMT+1"),
  to_utc_timestamp($"current_timestamp", "GMT+2")
)
