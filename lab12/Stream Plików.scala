...
// MAGIC %fs rm -r dbfs:/FileStore/streamDir

// COMMAND ----------

val nums = Seq.range(1,41,1)

for (x<-nums) dbutils.fs.cp(s"databricks-datasets/structured-streaming/events/file-$x.json", s"dbfs:/FileStore/streamDir/file-$x.json")
...
val streamingCountsDF = 
  streamingInputDF.groupBy($"action", window($"time", "1 hour")).count()
...
val query =streamingCountsDF.writeStream.outputMode("complete").format("memory").queryName("zliczenia").start()
...
// MAGIC val nums = Seq.range(41,50,1)
// MAGIC 
// MAGIC for (x<-nums) dbutils.fs.cp(s"databricks-datasets/structured-streaming/events/file-$x.json", s"dbfs:/FileStore/streamDir/file-$x.json")
