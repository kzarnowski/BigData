...
val analizaDF = twittsDF
 ...
 $"json.hashTags".as("hashTags"),                                          
   ...
   $"json.place.countryCode".as("countryCode") )import org.apache.spark.sql.functions.{from_json, expr}
...
val nazwaQuery = najpopularniejszeHashtagiDF
  .writeStream
  .format("memory")
  .outputMode("complete")
  .queryName("query_hashtag")
